...
import me.zygotecode.hycommonlib.eventapi.events.HsEventStoppable;
...
public final class HsEventManager {
     ...
     */
    public static void register(Object object, Class<? extends HsEvent> eventClass) {
        for (final Method method : object.getClass().getDeclaredMethods()) {
     ...
     */
    public static void unregister(Object object, Class<? extends HsEvent> eventClass) {
    ...
    private static void register(Method method, Object object) {
    	Class<? extends HsEvent> indexClass = (Class<? extends HsEvent>) method.getParameterTypes()[0];
    	//New MethodData from the Method we are registering.
    	final MethodData data = new MethodData(object, method, method.getAnnotation(HsEventTarget.class).value());
     ...
     */
    public static void removeEntry(Class<? extends HsEvent> indexClass) {
        Iterator<Map.Entry<Class<? extends HsEvent>, List<MethodData>>> mapIterator = REGISTRY_MAP.entrySet().iterator();
     ...
     */
    private static void sortListValue(Class<? extends HsEvent> indexClass) {
        List<MethodData> sortedList = new CopyOnWriteArrayList<MethodData>();

        for (final byte priority : HsPriority.VALUE_ARRAY) {
    ...
    private static boolean isMethodBad(Method method) {
        return method.getParameterTypes().length != 1 || !method.isAnnotationPresent(HsEventTarget.class);
    }
     ...
     * @see HsEventTarget
     */
    private static boolean isMethodBad(Method method, Class<? extends HsEvent> eventClass) {
     ...
     */
    public static final HsEvent call(final HsEvent hsEvent) {
        List<MethodData> dataList = REGISTRY_MAP.get(hsEvent.getClass());

        if (dataList != null) {
            if (hsEvent instanceof HsEventStoppable) {
                HsEventStoppable stoppable = (HsEventStoppable) hsEvent;
     ...
     * TODO: Error messages.
     */
    private static void invoke(MethodData data, HsEvent argument) {
        try {
